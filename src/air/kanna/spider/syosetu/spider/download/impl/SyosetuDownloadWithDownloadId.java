package air.kanna.spider.syosetu.spider.download.impl;
...
import air.kanna.spider.syosetu.spider.download.BaseSyosetuNovelDownloader;
...
import air.kanna.spider.syosetu.spider.util.Timer;
...
public class SyosetuDownloadWithDownloadId 
	...
	@Override
	public String download(SourceDataGetter getter, SyosetuNovel novel, File path, int model, int maxLength){
		...
		if(novel.getNovelContent().size() == 1
				&& novel.getNovelContent().get(0)
					.getChapterTitle().equalsIgnoreCase(
							SyosetuNovelSpider.DEFAULT_CHAPTER_TITLE)){
	...
	private List<Entry> getNovelStringByChapter(
		...
		for(int i=0; i<novel.getNovelContent().size(); i++){
			...
			for(int k=0; k<chapter.getChapterContent().size(); k++){
				...
				content.append(SyosetuNovelSpider.ENTER)
					.append(SyosetuNovelSpider.ENTER)
					.append(SyosetuNovelSpider.ENTER);
			}
	...
	protected String getSectionString(SourceDataGetter getter, SyosetuNovel novel, NovelSection section){
